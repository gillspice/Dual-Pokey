...
//  Modified for Arduino Micro and 1.79 Mhz
...
const byte CS0chip0pin = 5;
const byte CS0chip1pin = 13;
...
void setup() {
	...
	// Sets some of PORTF as outputs.
	// 
	// Connected to A0-A3 on POKEY
	DDRF = DDRF | B11110000; // set the direction for PORTF
...
void placeRegister(byte registerAddress) {
	...
	PORTF = (registerAddress & B11110000);
}
...
void writeChannelControl(byte dataType, byte data, 
	...
	static byte registerAddress[4] = {0x80,0xC0,0xA0,0xE0};
...
void writeFrequency(float frequency, byte channel, byte chipNumber) {
	...
	byte registerAddress[4] = {0,0x40,0x20,0x60};
	unsigned long halfPeriodInClockCycles;

	if (chipDivider == HIGH) {
		halfPeriodInClockCycles = 31961.0 / (float)frequency;
...
void writeForceAmplitude(byte amplitudeSetting, byte channel, byte chipNumber) {
	...
	byte registerAddress[4] = {0x80,0xC0,0xA0,0xE0};
